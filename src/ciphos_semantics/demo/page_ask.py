...
click, never cached across reruns, per `semantic_query.retrieve`.
"""
...
def _run_sql(warehouse: WarehouseConnection, sql: str) -> dict:
    try:
        rows = query_rows(warehouse.workspace, warehouse.warehouse_id, sql)
...
def render(
    ...
    semantic_store: SemanticStoreConnection,
) -> None:
    st.title("Ask")
    st.caption("Retrieve from the semantic map, generate grounded SQL and Cypher, then run them.")
    ...
    if st.button("Ask", type="primary", disabled=not question):
        try:
            with st.spinner("Retrieving from the semantic map…"):
                trace = retrieve(
                    question,
                    catalog=warehouse.catalog,
                    schema=warehouse.schema,
                    allowed_tables=indexed_table_names(),
    ...
    for tool_call in (trace.table_search, trace.column_search, trace.graph_context):
