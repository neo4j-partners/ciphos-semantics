...
retrieval tool call each came from. The Ask page never parses the generated
SQL or Cypher to check this: it takes the set of names semantic retrieval
returned and the set the model declared, and the difference is the panel. A
declared name absent from the retrieved set is a real finding (the model
reached for something the map did not give it), not a bug in the app.
"""
